...
| `/prototype/operations` | **The Operations redesign mock-up for Kaleb's review (Blueprint v5 §8, D15).** Second version. Left sidebar with counts; Today with an at-a-glance line, five groups, Done, snooze, delegate and pin; Relationships as a sortable table with a side panel and a full page; the journey as a workspace with a stage track and tabs; Transactions as a workstream grid; Offers, Calendar, Search, Advocacy, Reports, Questions and Settings sketched; Cmd+K and keyboard shortcuts. A "What changed" list on the page. Made-up data; nothing is saved |
