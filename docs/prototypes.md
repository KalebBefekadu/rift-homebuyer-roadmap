...
| `/prototype/operations` | **The Operations redesign mock-up for Kaleb's review (Blueprint v5 §8, D15).** Third version. Left sidebar with counts; Today as one "Needs you" list with the day and the coordinator's list beside it, the rest folded; Relationships as a sortable table with a side panel whose full page is the journey; the journey as a stage track where each stage is a checklist saying who does each step (Rift on its own, Rift for your approval, you, the coordinator, the client, a professional), from `lib/prototype/ops-playbook.ts`; Transactions as a workstream grid; Offers as one card per offer; Settings with sections, switches and a save bar, including Team, Automation and Checklists; Reviews and referrals, Reports, Lead-form questions, Calendar and Search; Cmd+K and keyboard shortcuts. A "What changed" list on the page. Made-up data; nothing is saved Built for real so far (26 Sep): the sidebar, Today, the journey with its checklist, Relationships with the side panel, Transactions, Offers, Settings (decisions, who does each step, team) and the coordinator's page at `/operations/tasks`; keep the mock-up until the rest (Cmd+K, snooze and delegation, the other Settings sections) is built and checked against it |
