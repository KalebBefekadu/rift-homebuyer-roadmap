...
| `team_members` | People the agent let in to record their part of the checklist. One role: coordinator | Blueprint v5 §8.7. Invited by a lower-case email, live once per agent; the first sign-in binds `auth_user_id` and `accepted_at` together (check). Removed with a reason, never deleted: a mark they made refers to them (`RESTRICT`). No client or coordinator RLS policy: coordinators read and write through the server, which resolves the membership every request. Migration `20260929000000` |
| `step_assignments` | Who does each checklist step, where the agent changed it from the default | Blueprint v5 §8.6. One row per step, with who changed it and when; no row is the default. Only `you`, `tc` or `rift`, and the writer refuses protected steps and the client's and professionals'. Migration `20260929000000` |
...
| `step_marks` | What was recorded against one checklist step of one journey: done (who and the day), reported, not needed (why), reopened (why) | Blueprint v5 §8.6. History only; the latest row per step is where it stands; `unique (journey_id, step_id, seq)` is the expected version. Workstream steps are not marked here: their state is `workstream_updates`. `actor_kind` and `team_member_id` say whether the agent or a named coordinator recorded it (check: a coordinator's mark names the coordinator). Written only by `lib/db/checklist.ts`. Migrations `20260928000000`, `20260929000000` |
| `reconciliations` | Each pilot check that Rift's record of a buyer's Matrix search and contract dates still matches Matrix and the documents | Blueprint v4 W12. History only. A checked search names the live package it was checked against, on the same journey (`search_packages` gained `unique (id, journey_id)` for this); a difference says what it was; a check of nothing is refused. What there is to check is worked out on the server. Written only by `lib/db/pilot.ts`. Migration `20260925010000` |
