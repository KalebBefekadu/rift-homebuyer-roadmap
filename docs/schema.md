...
| `job_runs` | Each run of a scheduled job and how it ended | Blueprint v4 W09. Server only (RLS on, no policies). Written by `trackedCron` in `lib/db/jobs.ts`; a refused or dry run is not recorded. Detail never holds an address |
