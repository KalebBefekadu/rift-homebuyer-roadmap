...
| LEAD-04 | Answers, preferences, source and snapshots carry into the client journey with their dates. Clients confirm only what is stale, missing, conflicting or consequential. | Built for the search brief: the newer of the readout and the saved plan starts the brief, each criterion dated and sourced ("saved plan of 2026-09-25") and "Not decided" until confirmed. Other answers (savings, income) are not carried: nothing in the brief uses them |
...
| MONEY-05 | A comfort range is a planning scenario, never a lending decision. An "how much can I afford" solver needs its own bounds, tests and disclosures before release. | Built: `/buy/afford`, two named planning scenarios (28/36 and 43), solved against the monthly-cost engine, bounded, tested (lib/core/afford.test.ts), and labelled "not a lending decision" beside the figure |
