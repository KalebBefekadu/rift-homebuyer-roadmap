        ...
        <ul style={{ marginTop: 8, display: "grid", gap: 4 }}>
          {docs.map((d) => (
            <li key={d.id} className="t-xs between gap-2 wrap">
              ...
              <a className="u" href={`/api/studio/document?journeyId=${journeyId}&id=${d.id}`} target="_blank" rel="noreferrer">Open</a>
            </li>
          ))}
        </ul>
      ) : null}
      ...
      {currentDocs.length ? (
        <p className="t-2xs c-3" style={{ marginTop: 4 }}>
          Documents: {currentDocs.map((id, i) => (
            <span key={id}>{i ? ", " : ""}<a className="u" href={`/api/studio/document?journeyId=${journeyId}&id=${id}`} target="_blank" rel="noreferrer">{docName.get(id) ?? "a document"}</a></span>
          ))}
        </p>
      ) : null}
