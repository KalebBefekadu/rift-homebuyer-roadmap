        ...
        {!on ? (
          ...
          <div className="card p-4" style={{ marginTop: 20 }}>
            ...
            <p className="t-xs c-3" style={{ marginTop: 6, lineHeight: 1.6 }}>
              ...
              <Link className="u" href="/operations/clients">Relationships</Link>.
            </p>
          </div>
        ) : (
          <>
            ...
            {owed.length ? (
              <div className="card p-3" style={{ marginTop: 12 }}>
                ...
                <ul className="t-xs" style={{ marginTop: 6, display: "grid", gap: 4 }}>
                  {owed.map((a, i) => (
                    <li key={i}>
                      <Link className="u" href={`/operations/journey/${a.journeyId}`}>{a.person}</Link>: {ASK_LABEL[a.kind].toLowerCase()}, asked {WHEN(a.askedAt)}
                ...
                {current.map((j) => {
                  ...
                  return (
                    <li key={j.id} className="card p-3">
                      <div className="between gap-2 wrap">
                        <span>
                          <Link className="t-sm w6 u" href={`/operations/journey/${j.id}`}>{j.person}</Link>
